...
export const GreenhouseView: React.FC<GreenhouseViewProps> = ({ plants, onSelectPlant, onExit, onOpenMixingStation, onOpenShop }) => {
    return (
        <div className="relative w-[800px] h-[600px] transition-all duration-500" style={{ transformStyle: 'preserve-3d', transform: 'rotateX(60deg) rotateZ(-45deg)' }}>
            ...
            {/* Back Wall */}
            <div className="absolute bottom-0 left-0 w-full h-48 bg-stone-300/80 transform origin-bottom" style={{ transform: 'rotateX(90deg)' }}></div>
             {/* Left Wall */}
            <div className="absolute top-0 left-0 w-48 h-full bg-stone-300/70 transform origin-top-left" style={{ transform: 'rotateY(-90deg)' }}></div>
